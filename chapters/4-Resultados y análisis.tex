...
Durante la auditoría estructural de los archivos planos anuales de GRD descargados desde la plataforma de datos abiertos, se determinó la codificación de caracteres de cada archivo, la presencia de anomalías de dimensionalidad en filas específicas (filas con un mayor número de columnas que el resto) y las modificaciones en las variables identificadoras de los pacientes. Estos resultados se resumen en la Tabla \ref{tab:auditoria_inicial_grd}:
...
\caption{Resultados de la auditoría estructural de los archivos brutos GRD (2019--2024).}
...
\parbox[t]{2.0cm}{\texttt{utf-8} (confianza: 0.99)} &
\parbox[t]{2.5cm}{1.151.475 / 129} &
\parbox[t]{5.2cm}{No se detectaron filas defectuosas. Se preservó el identificador histórico \texttt{CIP\_ENCRIPTADO}.} \\ \hline

\parbox[t]{1cm}{2020} &
\parbox[t]{2.3cm}{397.026.707} &
\parbox[t]{2.0cm}{\texttt{utf-8} (confianza: 0.99)} &
\parbox[t]{2.5cm}{781.912 / 129} &
\parbox[t]{5.2cm}{Disminución del volumen de registros, probablemente debido al impacto sanitario de la pandemia. Se mantuvo la consistencia del esquema de columnas.} \\ \hline

\parbox[t]{1cm}{2021} &
\parbox[t]{2.3cm}{428.657.825} &
\parbox[t]{2.0cm}{\texttt{utf-8-sig} (confianza: 1)} &
\parbox[t]{2.5cm}{816.909 / 129} &
\parbox[t]{5.2cm}{Se incorporó la marca de orden de bytes (BOM) al inicio del archivo. La lectura se realizó sin pérdida de información.} \\ \hline

\parbox[t]{1cm}{2022} &
\parbox[t]{2.3cm}{960.774.530} &
\parbox[t]{2.0cm}{\texttt{utf-16} (confianza: 1)} &
\parbox[t]{2.5cm}{932.840 / 129} &
\parbox[t]{5.2cm}{Migración de la codificación a \texttt{utf-16}. Además, se detectó una anomalía posicional en la línea 120.127, con 130 columnas en lugar de 129, la cual fue corregida durante el preprocesamiento.} \\ \hline

\parbox[t]{1cm}{2023} &
\parbox[t]{2.3cm}{1.075.450.968} &
\parbox[t]{2.0cm}{\texttt{utf-16} (confianza: 1)} &
\parbox[t]{2.5cm}{1.039.587 / 129} &
\parbox[t]{5.2cm}{Incremento del volumen de registros, superando el millón de egresos. No se detectaron problemas de delimitación en el archivo.} \\ \hline

\parbox[t]{1cm}{2024} &
\parbox[t]{2.3cm}{566.268.922} &
\parbox[t]{2.0cm}{\texttt{latin1} (\texttt{ISO-8859-1}) (confianza: 0.73)} &
\parbox[t]{2.5cm}{1.085.813 / 129} &
\parbox[t]{5.2cm}{La detección automática obtuvo una confianza de 0.73 para la codificación \texttt{ISO-8859-1}. La lectura se estabilizó mediante decodificación manual con \texttt{latin1}. Además, el identificador del paciente cambió de \texttt{CIP\_ENCRIPTADO} a \texttt{ID\_BENEFICIARIO}.} \\ \hline

\end{tabular}
\end{table}

El análisis comparativo determinó que todas las columnas mantuvieron una equivalencia funcional a lo largo de los seis años analizados y conservaron sus nombres, con excepción del identificador del paciente en 2024, que cambió de \texttt{CIP\_ENCRIPTADO} a \texttt{ID\_BENEFICIARIO}. Sin embargo, esta inconsistencia fue mitigada durante la etapa de clasificación oncológica para preservar la continuidad del análisis temporal.

Respecto de la codificación de caracteres, el archivo correspondiente a 2024 presentó una confianza inferior a 0.99 en la detección automática. Por este motivo, se evaluó su lectura utilizando distintas codificaciones, entre ellas \texttt{utf-8}, \texttt{utf-16}, \texttt{cp1252} y \texttt{latin1}. Posteriormente, se realizó un conteo de caracteres especiales (como Ñ, ñ y vocales acentuadas) bajo cada alternativa para verificar que dichos caracteres se preservaran correctamente. Finalmente, aunque el detector automático identificó la codificación como \texttt{ISO-8859-1}, se optó por utilizar \texttt{latin1}, dado que ambas son equivalentes en Python, ya que mapean los bytes del 0 al 255 a los mismos caracteres. Además, \texttt{latin1} constituye la codificación más utilizada en scripts de Python para este tipo de archivos, lo que favorece la compatibilidad y garantiza la correcta preservación de caracteres acentuados y de la letra ñ.

Cabe destacar que los años 2019 y 2024 presentaron un mayor número de episodios (filas) que los años intermedios. Por otro lado, durante 2020 y 2021 se observó una disminución del volumen de registros, probablemente como consecuencia del impacto sanitario asociado a la pandemia de COVID-19.

\subsection{Distribución oncológica y control de la cohorte}

Tras el filtrado inicial y la clasificación de diagnósticos, el proceso de limpieza y unificación permitió retener un total de $5.808.455$ episodios clínicos válidos. Al aplicar el algoritmo de clasificación basado en el Capítulo II de la CIE-10 (C00-C97), se detectaron $498.618$ episodios que presentaban al menos un diagnóstico oncológico activo durante su estadía hospitalaria, ya fuera en el diagnóstico principal (\texttt{DIAGNOSTICO1}) o en alguno de los diagnósticos secundarios (\texttt{DIAGNOSTICO2-35}).

Respecto de los episodios oncológicos identificados, se observaron diferencias entre los motivos de ingreso directo (diagnóstico principal) y las comorbilidades oncológicas (diagnósticos secundarios). En particular, el análisis reveló que el 4,68\% de los episodios hospitalarios presentaban un diagnóstico oncológico principal ($272.093$ instancias), mientras que el 3,90\% correspondían a cáncer como diagnóstico secundario ($226.525$ instancias). En conjunto, la cohorte oncológica representó el 8,58\% de todos los egresos del período, mientras que el grupo de control (\texttt{SIN\_CANCER}) estuvo conformado por $5.309.837$ episodios (91,42\%).

Como se observa en la Tabla \ref{tab:distribucion_anual_cohorte}, el año 2024 presentó el mayor número de episodios oncológicos respecto del resto de los años analizados, con un total de $55.111$ episodios con cáncer como diagnóstico principal y $44.042$ como diagnóstico secundario. En conjunto, estos representaron el 9,14\% de los episodios hospitalarios de ese año, alineándose con la tendencia creciente de la carga oncológica descrita en los antecedentes de esta investigación. En contraste, el año 2020 registró la menor cantidad de episodios oncológicos, con $35.294$ episodios con cáncer como diagnóstico principal y $29.632$ como diagnóstico secundario, equivalentes al 8,30\% de los episodios hospitalarios del período, probablemente como consecuencia del impacto sanitario asociado a la pandemia de COVID-19.
...
\rowcolor{gray!20}
\parbox[t]{1.5cm}{\centering\textbf{TOTAL}} & \parbox[t]{1.8cm}{\raggedleft\textbf{272.093}} & \parbox[t]{1.5cm}{\raggedleft\textbf{4,68\%}} & \parbox[t]{1.8cm}{\raggedleft\textbf{226.525}} & \parbox[t]{1.5cm}{\raggedleft\textbf{3,90\%}} & \parbox[t]{2.0cm}{\raggedleft\textbf{5.309.837}} & \parbox[t]{1.5cm}{\raggedleft\textbf{91,42\%}} & \parbox[t]{1.8cm}{\raggedleft\textbf{5.808.455}} \\ \hline
\end{tabular}
\end{table}

Por otro lado, la Figura \ref{fig:proporcion_total} muestra la distribución de los diagnósticos oncológicos en la población general, evidenciando que la mayoría de los episodios oncológicos correspondieron a diagnósticos principales (4,68\%), en comparación con los diagnósticos secundarios (3,90\%). Este resultado sugiere que el cáncer constituye con frecuencia la causa principal de hospitalización, aunque también es habitual que los pacientes ingresen por otras patologías y presenten cáncer como comorbilidad. En conjunto, la cohorte oncológica representó el 8,58\% de todos los egresos hospitalarios, lo que evidencia la importante carga asistencial que el cáncer impone sobre el sistema público de salud chileno. Asimismo, tal como se esperaba, la mayor proporción de episodios correspondió a pacientes sin diagnóstico oncológico (grupo de control, \texttt{SIN\_CANCER}), representando el 91,42\% de los episodios hospitalarios. La incorporación de este grupo de control de gran tamaño resulta fundamental para minimizar sesgos de selección y permitir comparaciones robustas entre pacientes oncológicos y no oncológicos en las etapas posteriores del análisis, especialmente en la evaluación de factores de riesgo asociados al cáncer.

\begin{figure}[htbp]
    \centering
	\captionsetup{justification=centering}
    \includegraphics[width=0.5\textwidth]{images/proporcion_total_pacientes.png}
...
Finalmente, se elaboró un gráfico de barras para visualizar la distribución de cada tipo de cáncer según la clasificación CIE-10 (Figura \ref{fig:barras_categorias}), en donde se observa que la mayor cantidad de episodios oncológicos se concentró en los cánceres de órganos digestivos (C15-C26), con $133.249$ episodios, seguidos por las neoplasias de tejidos linfoide, hematopoyético y relacionados (C81-C96), con $85.492$ episodios, el cáncer de mama (C50), con $59.990$ episodios, y los tumores de órganos genitales femeninos (C51-C58), con $42.030$ episodios. Por ende, esta distribución confirma la elevada demanda de recursos que estas localizaciones anatómicas generan dentro del sistema hospitalario chileno, lo que tiene implicaciones directas para la planificación de los servicios oncológicos y la asignación de recursos sanitarios.

\begin{figure}[htbp]
    \centering
	\captionsetup{justification=centering}
    \includegraphics[width=0.8\textwidth]{images/barras_total_pacientes.png}
    \caption{Distribución de episodios según categoría CIE-10 frente al grupo de control.
    \\Fuente: Elaboraci\'on propia, 2026.}
    \label{fig:barras_categorias}
\end{figure}

\subsection{Cardinalidad y juicio sobre las variables identificadoras para la fase de modelado}

Tras analizar la cardinalidad y la distribución de cada variable identificadora en los registros GRD, se consolidó la Tabla \ref{tab:cardinalidad_identificadores}, la cual presenta el volumen de identificadores únicos para pacientes, hospitales y médicos. En ella se observa que todos los identificadores presentan una alta cardinalidad, destacando el identificador de pacientes (\texttt{CIP\_ENCRIPTADO}), que registró entre 98.379 y 165.128 pacientes únicos durante el período analizado. Asimismo, se identificaron entre 65 y 72 hospitales únicos por año y más de 9.476 médicos distintos en cada período.

Respecto al análisis de reingresos de pacientes, presentado en la Tabla \ref{tab:reingresos_pacientes}, se observa que durante los años 2019 y 2020 la frecuencia máxima estuvo dominada artificialmente por la etiqueta \texttt{SIN INFORMACIÓN}, registrando $8.307$ y $1.893$ episodios, respectivamente. Este hallazgo evidencia una limitación en la codificación de pacientes de la base de datos de origen, ya que el sistema agrupó bajo una misma cadena de texto (\texttt{SIN INFORMACIÓN}) todos aquellos egresos hospitalarios cuyos datos de identificación civil no pudieron ser encriptados o resultaron ilegibles en el nodo central. Por otra parte, a partir del año 2021 destacó el identificador anónimo \texttt{95162030}, el cual registró la mayor cantidad de hospitalizaciones durante tres períodos consecutivos, con un mínimo de 294 reingresos en 2021 y un máximo de 523 episodios en 2022.

En relación con los hospitales, la Tabla \ref{tab:concentracion_hospitales} evidencia que, aunque la cantidad de establecimientos hospitalarios aumentó de 65 en 2019 a 72 en 2024, la distribución de la carga asistencial fue marcadamente asimétrica. De forma consistente durante el período analizado, los cinco establecimientos de mayor complejidad concentraron entre el 16,5\% y el 18,6\% del total de egresos hospitalarios de la red pública nacional. Entre ellos, el establecimiento identificado con el código \texttt{114101} lideró sistemáticamente este indicador de volumen asistencial, correspondiente al Complejo Hospitalario Dr. Sótero del Río, ubicado en la comuna de Puente Alto, Santiago.

Además del hospital \texttt{114101}, entre los establecimientos con mayor frecuencia de registros se encontraron los hospitales codificados como \texttt{106100} (Hospital Carlos Van Buren, Valparaíso), \texttt{118100} (Hospital Clínico Regional Dr. Guillermo Grant Benavente, Concepción), \texttt{109100} (Complejo Hospitalario San José, Santiago, Independencia), \texttt{121109} (Hospital Dr. Hernán Henríquez Aravena, Temuco), \texttt{113100} (Hospital Barros Luco Trudeau, Santiago, San Miguel), \texttt{116105} (Hospital Dr. César Garavagno Burotto, Talca), \texttt{120101} (Complejo Asistencial Dr. Víctor Ríos Ruiz, Los Ángeles) y \texttt{124105} (Hospital de Puerto Montt). Cabe mencionar que estas codificaciones se encuentran especificadas en las tablas maestras GRD disponibles en el visor de datos abiertos de FONASA (\cite{fonasa_visor_grd}).

En base a la cardinalidad y la distribución de frecuencias de cada identificador, se adoptaron las siguientes decisiones metodológicas respecto de su utilización en la fase de modelado:

\begin{enumerate}
    \item \textbf{Identificador de paciente (\texttt{CIP\_ENCRIPTADO} o \texttt{ID\_BENEFICIARIO}) (variable demográfica):} Se \textbf{descartó} su utilización en la fase de modelado, ya que el identificador encriptado de cada paciente no presenta una relación clínica ni operativa con su nivel de criticidad. Además, se identificaron pacientes con volúmenes de hospitalización sospechosamente elevados o humanamente improbables, como los 523 episodios correspondientes al identificador \texttt{95162030} en 2022. Esto sugiere que la variable incorpora identificadores institucionales o códigos por defecto utilizados para pacientes sin registro. En consecuencia, la aplicación de transformaciones como \textit{One-Hot Encoding} sobre una variable con miles de categorías generaría un conjunto de datos altamente dimensional y propenso al sobreajuste (\textit{overfitting}), afectando negativamente el desempeño de los algoritmos de selección de características. Asimismo, cada episodio se evaluó como una instancia independiente según su contexto clínico.
    \item \textbf{Identificador de hospital (\texttt{COD\_HOSPITAL}) (variable hospitalaria):} Se \textbf{descartó} su utilización en la fase de modelado debido a su elevada cardinalidad (entre 65 y 72 categorías por año). Además, esta variable se encuentra jerárquicamente contenida dentro de la variable \texttt{SERVICIO\_SALUD}, la cual agrupa los establecimientos según macrozonas geográficas y presenta una menor cardinalidad. Por lo tanto, mantener ambas variables incorporaría información redundante al modelo. Adicionalmente, el objetivo principal de este estudio consiste en apoyar la planificación sanitaria y la gestión de recursos a nivel macro, orientadas a organismos como FONASA y el MINSAL. En este contexto, un análisis centrado en hospitales individuales excede el alcance epidemiológico de la caracterización propuesta.
    \item \textbf{Identificador del médico de la primera intervención (\texttt{MEDICOINTERV1\_ENCRIPTADO}) (variable hospitalaria):} Se \textbf{descartó} su utilización en la fase de modelado, ya que registra hasta 11.164 valores únicos por año, lo que generaría una matriz altamente dispersa y penalizaría el desempeño de los algoritmos de selección de características. Por esta razón, su incorporación resulta metodológicamente inviable. Además, el objetivo del estudio consiste en caracterizar aspectos clínicos y asistenciales, tales como los procedimientos realizados, y no identificar a los profesionales que los ejecutaron.
    \item \textbf{Identificador del médico de alta (\texttt{MEDICOALTA\_ENCRIPTADO}) (variable hospitalaria):} Se \textbf{descartó} su utilización en la fase de modelado debido a su cardinalidad extremadamente alta, alcanzando hasta 17.743 valores únicos por año. Esto generaría una matriz dispersa que afectaría negativamente el desempeño de los algoritmos de selección de características, haciendo inviable su incorporación desde un punto de vista metodológico. En términos prácticos, la transformación de esta variable sería computacionalmente ineficiente y desviaría al modelo de su objetivo principal, que consiste en identificar perfiles clínicos, demográficos y hospitalarios a nivel de la red pública de salud.
\end{enumerate}
...
\parbox[t]{2.5cm}{\raggedright\texttt{CIP\_ENCRIPTADO}\\\textit{(Pacientes únicos repetidos)}} & \parbox[t]{1.8cm}{\raggedleft 165.128} & \parbox[t]{1.8cm}{\raggedleft 98.379} & \parbox[t]{1.8cm}{\raggedleft 108.690} & \parbox[t]{1.8cm}{\raggedleft 129.107} & \parbox[t]{1.8cm}{\raggedleft 151.694} & \parbox[t]{1.8cm}{\raggedleft 161.533} \\ \hline
\parbox[t]{2.5cm}{\raggedright\texttt{COD\_HOSPITAL}\\\textit{(Hospitales emisores)}} & \parbox[t]{1.8cm}{\raggedleft 65} & \parbox[t]{1.8cm}{\raggedleft 65} & \parbox[t]{1.8cm}{\raggedleft 65} & \parbox[t]{1.8cm}{\raggedleft 65} & \parbox[t]{1.8cm}{\raggedleft 68} & \parbox[t]{1.8cm}{\raggedleft 72} \\ \hline
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.2}
\caption{Casos críticos de reingreso hospitalario (2019-2024).}
...
\subsection{Juicio sobre las de variables temporales para la fase de modelado}

El análisis de las variables de tipo fecha constituyó un paso crítico para evaluar la coherencia cronológica de los registros asistenciales y asegurar la correcta construcción posterior de atributos derivados (como la edad y los días de estancia hospitalaria). Los indicadores de formato, cantidad de valores nulos, registros inválidos y rangos temporales identificados se presentan en la Tabla \ref{tab:auditoria_fechas_grd}.

En esta auditoría se detectó que las fechas de nacimiento e ingreso presentaron una tasa mínima de valores nulos (0\%), lo que demuestra que constituyen campos obligatorios validados por los sistemas informáticos hospitalarios de admisión de la red pública. Sin embargo, se confirmó una alteración en el formato de registro del año 2023, en el cual se adoptó el estándar \texttt{dd-mm-yyyy}, en contraste con el formato ISO \texttt{yyyy-mm-dd} utilizado de manera homogénea en el resto de los años. Por otra parte, las fechas de traslado exhibieron un volumen creciente de valores nulos, que osciló entre el 82,77\% y el 99,98\%. Esto sugiere que el traslado interno entre múltiples servicios especializados (como la unidad de tratamiento intermedio (UTI), la unidad de cuidados intensivos (UCI) o cirugía) refleja directamente la inestabilidad o severidad del paciente y ocurre únicamente en una fracción minoritaria de los ingresos asistenciales. En consecuencia, la mayor parte de la población concentró su estancia en un único servicio base, por lo que la mayoría de las fechas de traslado presentan cerca de un 100 \% de valores nulos.

Otro hallazgo crítico se asoció a la fecha de procedimiento, la cual presentó una elevada tasa de valores nulos y registros corruptos, que alcanzó casi la totalidad de los episodios ($5.808.446$), coexistiendo con formatos irreconocibles fuera del año 2019. Por otro lado, respecto de la fecha de intervención, se detectó una elevada tasa de valores nulos (49\%, correspondientes a $2.873.826$ registros) y un total de $19.684$ registros inválidos, con una distorsión temporal que se extendió de manera inconsistente hasta el año 2090. Esto evidencia errores sistemáticos de digitación por parte de los operadores de la red de salud al ingresar las fechas de las intervenciones en los sistemas locales.

Es fundamental destacar que la totalidad de las características analizadas, con excepción de \texttt{FECHAPROCEDIMIENTO1}, registró un rango válido de meses estrictamente acotado entre 1 y 12, así como un rango de días comprendido entre 1 y 31. Esto confirmó que la estructura granular de los datos temporales era matemáticamente válida, legitimando posteriormente el cálculo de variables derivadas, como la edad y los días de estancia hospitalaria.

Finalmente, se tomaron las siguientes decisiones para cada variable temporal:
\begin{enumerate}
    \item \textbf{Fecha de nacimiento (\texttt{FECHA\_NACIMIENTO}) (variable demográfica) y fecha de ingreso (\texttt{FECHA\_INGRESO}) (variable hospitalaria):} Estas variables se \textbf{descartaron} en su formato original, pero se destinaron a la generación de la variable \textbf{derivada} \texttt{EDAD\_INGRESO}, ya que ambas presentan un 0\% de valores nulos reales en toda la cohorte. Además, aunque se detectó un quiebre sistémico en el formato de las fechas durante 2023 (cambio de \texttt{yyyy-mm-dd} a \texttt{dd-mm-yyyy}), este se puede resolver mediante un proceso de parseo robusto. Asimismo, si bien los algoritmos basados en árboles (como Random Forest y XGBoost) no procesan de forma nativa variables de tipo fecha (\textit{datetime}), la diferencia matemática entre ambas fechas permite calcular la \textbf{edad} del paciente al ingreso hospitalario, un determinante demográfico potencialmente relevante para la evaluación del riesgo crítico en oncología.
    \item \textbf{Fechas de traslado (\texttt{FECHATRASLADO1} hasta \texttt{FECHATRASLADO9}) (variables hospitalarias):} Estas variables se \textbf{descartaron} completamente debido a su elevado nivel de datos faltantes. En otras palabras, las fechas de traslado no aportan valor predictivo directo, ya que la primera fecha de traslado (\texttt{FECHATRASLADO1}) presenta un 82,77 \% de valores nulos, proporción que aumenta rápidamente hasta superar el 99 \% a partir de la cuarta fecha. Además, mantener estas variables en su formato original solo introduciría columnas prácticamente vacías, sin aportar capacidad de generalización a los modelos. Por ende, posteriormente se utilizará una variable categórica asociada a los traslados (\texttt{SERVICIO\_TRASLADO}) para generar la variable derivada \texttt{CANTIDAD\_TRASLADOS}, la cual sí puede aportar valor predictivo al capturar el número de traslados intrahospitalarios experimentados en cada episodio, constituyendo un indicador directo de inestabilidad clínica.
    \item \textbf{Fecha de alta (\texttt{FECHAALTA}) (variable hospitalaria):} Se \textbf{descartó} su uso en formato original, pero se utilizará para generar la variable \textbf{derivada} \texttt{DIAS\_ESTADIA}. Si bien esta fecha, por sí sola, no describe el perfil del paciente, la diferencia entre \texttt{FECHAALTA} y \texttt{FECHA\_INGRESO} permite estimar la duración de la estancia hospitalaria. Esta variable constituye potencialmente uno de los indicadores más influyentes en el consumo de recursos y en la severidad hospitalaria, por lo que resulta esencial para la caracterización de grupos oncológicos. Además, la calidad de los datos es excelente, con una tasa cercana al 0\% de valores nulos reales. No obstante, esta variable se encuentra sujeta al mismo quiebre de formato detectado en 2023 para la fecha de ingreso.
    \item \textbf{Fecha de procedimiento (\texttt{FECHAPROCEDIMIENTO1}) (variable hospitalaria):} Se \textbf{descartó} completamente para la fase de modelado, ya que esta columna se encuentra vacía o inválida en todos los archivos anuales analizados (2019--2024), por lo que carece de utilidad analítica y predictiva. 
    \item \textbf{Fecha de intervención (\texttt{FECHAINTERV1}) (variable hospitalaria):} Se \textbf{descartó} completamente su uso, ya que presenta una elevada tasa de valores nulos (49\%), probablemente asociada al porcentaje de pacientes que no requirieron intervención quirúrgica. Adicionalmente, esta variable exhibe un alto nivel de corrupción de datos, evidenciado por la presencia de valores atípicos e inconsistencias lógicas severas, como registros correspondientes al año 2090 en admisiones ocurridas en 2023 o fechas de intervención registradas en 1902 para episodios de 2022. Por consiguiente, la combinación de una baja densidad de datos y un alto nivel de corrupción introduciría ruido excesivo en los modelos.
\end{enumerate}
...
\parbox[t]{2.8cm}{\texttt{FECHA\_NACIMIENTO}} & \parbox[t]{3.6cm}{\centering \texttt{yyyy-mm-dd}} & \parbox[t]{2.2cm}{\raggedleft 0 (0,00\%)} & \parbox[t]{1.3cm}{\raggedleft 21} & \parbox[t]{1.8cm}{\centering 1894-2024} \\ \hline
...
\parbox[t]{2.8cm}{\texttt{FECHAINTERV1}} & \parbox[t]{3.6cm}{\centering \texttt{dd-mm-yyyy} (2023) / \texttt{yyyy-mm-dd}} & \parbox[t]{2.2cm}{\raggedleft 2.873.826 (49,48\%)} & \parbox[t]{1.3cm}{\raggedleft 19.684} & \parbox[t]{1.8cm}{\centering 1902-2090} \\ \hline
\end{tabular}
\end{table}

\subsection{Cardinalidad de las variables categóricas objetivo (targets)}
...
En primer lugar, dentro de los datos GRD se encuentran dos variables que describen la \textbf{mortalidad} hospitalaria: \texttt{IR\_29301\_MORTALIDAD} y \texttt{TIPOALTA}. Sin embargo, la primera corresponde a un índice sintético calculado por el sistema de codificación GRD, por lo que resulta redundante conservarla para la fase de modelado, ya que la variable \texttt{TIPOALTA} constituye un descriptor categórico directo de la condición de alta del paciente e incluye una categoría específica para la muerte hospitalaria. Es decir, esta variable representa la realidad (\textit{ground truth}) del desenlace clínico del paciente. De esta forma, el modelo aprenderá a partir del desenlace clínico observado (mortalidad real), en lugar de utilizar un índice de riesgo calculado por el sistema de codificación.

Como se mencionó, la variable \texttt{TIPOALTA} representa la verdad fundamental (\textit{ground truth}) del desenlace clínico de cada paciente (mortalidad). En el análisis de frecuencias presentado en las Tablas \ref{tab:frecuencia_tipo_alta_total}, \ref{tab:frecuencia_tipo_alta_onco} y \ref{tab:frecuencia_tipo_alta_control} se puede observar que la tasa de mortalidad de los pacientes oncológicos duplicó y, en ocasiones, triplicó la tasa de mortalidad de la cohorte control. En específico, para los pacientes con cáncer, la cantidad de episodios con desenlace de muerte hospitalaria osciló entre el 5,4\% y el 6,9\%, mientras que, para el grupo control, la tasa de mortalidad se mantuvo entre el 2,1\% y el 3,7\%.

Es de particular interés destacar la tasa de defunciones durante la pandemia de COVID-19 en los años 2020 y 2021, cuando la mortalidad oncológica alcanzó sus máximos históricos (6,9\% y 6,5\%, respectivamente), a pesar de que el volumen absoluto de decesos (4.462 y 4.423) fue menor que en años previos. Este fenómeno estadístico probablemente se deba a la contracción severa de los ingresos hospitalarios durante la pandemia, lo que concentró las admisiones exclusivamente en pacientes con neoplasias en estado crítico o terminal. Además, mientras que la mortalidad del grupo control regresó a sus valores prepandémicos de referencia en 2022 y 2023 (2,6\% y 2,3\%, respectivamente), la mortalidad oncológica se mantuvo elevada, por encima del 5,9\%, hasta 2024, lo que resulta consistente con las proyecciones de mortalidad para la población con cáncer.

Cabe mencionar que, la variable \texttt{TIPOALTA}, que originalmente contiene 11 categorías (domicilio, fallecido, hospitalización domiciliaria, derivación de otro hospital del servicio, alta voluntaria, derivación de otro hospital de la red nacional, derivación a otros centros (cárcel), fuga del paciente, derivación a institución privada (compra de servicios), derivación a institución privada (voluntario) y no identificada), se binarizará en una variable dicotómica (\texttt{MORTALIDAD}) con las clases \texttt{Fallecido} (1) y \texttt{Vivo} (0). En donde la clase negativa agrupará los episodios con desenlace de alta voluntaria, domicilio, derivaciones, entre otros, exceptuando las categorías marginales sin información (\texttt{NO IDENTIFICADA} y \texttt{DESCONOCIDO}), las cuales serán eliminadas debido a su frecuencia prácticamente nula (100 casos en total durante todo el período analizado).
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.4}
\caption{Distribución anual del tipo de alta para la cohorte total.}
\label{tab:frecuencia_tipo_alta_total}
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
\parbox[t]{4.0cm}{\centering\textbf{TIPO DE ALTA}} & 
\parbox[t]{1.2cm}{\centering\textbf{2019}} & 
\parbox[t]{1.2cm}{\centering\textbf{2020}} & 
\parbox[t]{1.2cm}{\centering\textbf{2021}} & 
\parbox[t]{1.2cm}{\centering\textbf{2022}} & 
\parbox[t]{1.2cm}{\centering\textbf{2023}} & 
\parbox[t]{1.2cm}{\centering\textbf{2024}} & 
\parbox[t]{1.6cm}{\centering\textbf{TOTAL}} \\ \hline
\parbox[t]{4.0cm}{\raggedright DOMICILIO} & \parbox[t]{1.2cm}{\raggedleft 1052020\\(91.4\%)} & \parbox[t]{1.2cm}{\raggedleft 694899\\(88.9\%)} & \parbox[t]{1.2cm}{\raggedleft 716270\\(87.7\%)} & \parbox[t]{1.2cm}{\raggedleft 838234\\(89.9\%)} & \parbox[t]{1.2cm}{\raggedleft 936205\\(90.1\%)} & \parbox[t]{1.2cm}{\raggedleft 967852\\(89.1\%)} & \parbox[t]{1.6cm}{\raggedleft 5205480\\(89.6\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright FALLECIDO} & \parbox[t]{1.2cm}{\raggedleft 29584\\(2.6\%)} & \parbox[t]{1.2cm}{\raggedleft 29942\\(3.8\%)} & \parbox[t]{1.2cm}{\raggedleft 31965\\(3.9\%)} & \parbox[t]{1.2cm}{\raggedleft 27379\\(2.9\%)} & \parbox[t]{1.2cm}{\raggedleft 25136\\(2.4\%)} & \parbox[t]{1.2cm}{\raggedleft 26682\\(2.5\%)} & \parbox[t]{1.6cm}{\raggedleft 170688\\(2.9\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright HOSPITALIZACION DOMICILIARIA} & \parbox[t]{1.2cm}{\raggedleft 12987\\(1.1\%)} & \parbox[t]{1.2cm}{\raggedleft 14071\\(1.8\%)} & \parbox[t]{1.2cm}{\raggedleft 22403\\(2.7\%)} & \parbox[t]{1.2cm}{\raggedleft 23408\\(2.5\%)} & \parbox[t]{1.2cm}{\raggedleft 29298\\(2.8\%)} & \parbox[t]{1.2cm}{\raggedleft 36732\\(3.4\%)} & \parbox[t]{1.6cm}{\raggedleft 138899\\(2.4\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION OTRO HOSPITAL DEL SERVICIO} & \parbox[t]{1.2cm}{\raggedleft 25206\\(2.2\%)} & \parbox[t]{1.2cm}{\raggedleft 17839\\(2.3\%)} & \parbox[t]{1.2cm}{\raggedleft 18321\\(2.2\%)} & \parbox[t]{1.2cm}{\raggedleft 16488\\(1.8\%)} & \parbox[t]{1.2cm}{\raggedleft 20505\\(2.0\%)} & \parbox[t]{1.2cm}{\raggedleft 24923\\(2.3\%)} & \parbox[t]{1.6cm}{\raggedleft 123282\\(2.1\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright ALTA VOLUNTARIA} & \parbox[t]{1.2cm}{\raggedleft 9963\\(0.9\%)} & \parbox[t]{1.2cm}{\raggedleft 7569\\(1.0\%)} & \parbox[t]{1.2cm}{\raggedleft 9059\\(1.1\%)} & \parbox[t]{1.2cm}{\raggedleft 10721\\(1.1\%)} & \parbox[t]{1.2cm}{\raggedleft 10317\\(1.0\%)} & \parbox[t]{1.2cm}{\raggedleft 10651\\(1.0\%)} & \parbox[t]{1.6cm}{\raggedleft 58280\\(1.0\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION OTRO HOSPITAL DE LA RED NACIONAL} & \parbox[t]{1.2cm}{\raggedleft 8415\\(0.7\%)} & \parbox[t]{1.2cm}{\raggedleft 6871\\(0.9\%)} & \parbox[t]{1.2cm}{\raggedleft 7194\\(0.9\%)} & \parbox[t]{1.2cm}{\raggedleft 6539\\(0.7\%)} & \parbox[t]{1.2cm}{\raggedleft 7472\\(0.7\%)} & \parbox[t]{1.2cm}{\raggedleft 7938\\(0.7\%)} & \parbox[t]{1.6cm}{\raggedleft 44429\\(0.8\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION A OTROS CENTROS (CARCEL)} & \parbox[t]{1.2cm}{\raggedleft 2980\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 4396\\(0.6\%)} & \parbox[t]{1.2cm}{\raggedleft 4038\\(0.5\%)} & \parbox[t]{1.2cm}{\raggedleft 3385\\(0.4\%)} & \parbox[t]{1.2cm}{\raggedleft 3497\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 3745\\(0.3\%)} & \parbox[t]{1.6cm}{\raggedleft 22041\\(0.4\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright FUGA DEL PACIENTE} & \parbox[t]{1.2cm}{\raggedleft 4141\\(0.4\%)} & \parbox[t]{1.2cm}{\raggedleft 2719\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 2593\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 3184\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 3224\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 3299\\(0.3\%)} & \parbox[t]{1.6cm}{\raggedleft 19160\\(0.3\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION INST. PRIVADA (COMPRA DE SERVICIOS)} & \parbox[t]{1.2cm}{\raggedleft 4411\\(0.4\%)} & \parbox[t]{1.2cm}{\raggedleft 2430\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 3688\\(0.5\%)} & \parbox[t]{1.2cm}{\raggedleft 2373\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 2886\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 2865\\(0.3\%)} & \parbox[t]{1.6cm}{\raggedleft 18653\\(0.3\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION INST. PRIVADA (VOLUNTARIO)} & \parbox[t]{1.2cm}{\raggedleft 1614\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 1152\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 1378\\(0.2\%)} & \parbox[t]{1.2cm}{\raggedleft 1126\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 1047\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 1126\\(0.1\%)} & \parbox[t]{1.6cm}{\raggedleft 7443\\(0.1\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright NO IDENTIFICADA} & \parbox[t]{1.2cm}{\raggedleft 58\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 22\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.6cm}{\raggedleft 80\\(0.0\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DESCONOCIDO} & \parbox[t]{1.2cm}{\raggedleft 19\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 1\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.6cm}{\raggedleft 20\\(0.0\%)} \\ \hline
\end{tabular}
\end{table}
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.4}
\caption{Distribución anual del tipo de alta para la cohorte oncológica.}
\label{tab:frecuencia_tipo_alta_onco}
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
\parbox[t]{4.0cm}{\centering\textbf{TIPO DE ALTA}} & 
\parbox[t]{1.2cm}{\centering\textbf{2019}} & 
...
\parbox[t]{1.2cm}{\centering\textbf{2024}} & 
\parbox[t]{1.6cm}{\centering\textbf{TOTAL}} \\ \hline
\parbox[t]{4.0cm}{\raggedright DOMICILIO} & \parbox[t]{1.2cm}{\raggedleft 88008\\(90.0\%)} & \parbox[t]{1.2cm}{\raggedleft 56334\\(86.8\%)} & \parbox[t]{1.2cm}{\raggedleft 58867\\(86.7\%)} & \parbox[t]{1.2cm}{\raggedleft 67703\\(86.7\%)} & \parbox[t]{1.2cm}{\raggedleft 78385\\(86.4\%)} & \parbox[t]{1.2cm}{\raggedleft 84680\\(85.4\%)} & \parbox[t]{1.6cm}{\raggedleft 433977\\(87.0\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright FALLECIDO} & \parbox[t]{1.2cm}{\raggedleft 5321\\(5.4\%)} & \parbox[t]{1.2cm}{\raggedleft 4462\\(6.9\%)} & \parbox[t]{1.2cm}{\raggedleft 4423\\(6.5\%)} & \parbox[t]{1.2cm}{\raggedleft 4850\\(6.2\%)} & \parbox[t]{1.2cm}{\raggedleft 5314\\(5.9\%)} & \parbox[t]{1.2cm}{\raggedleft 6009\\(6.1\%)} & \parbox[t]{1.6cm}{\raggedleft 30379\\(6.1\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright HOSPITALIZACION DOMICILIARIA} & \parbox[t]{1.2cm}{\raggedleft 1319\\(1.3\%)} & \parbox[t]{1.2cm}{\raggedleft 1471\\(2.3\%)} & \parbox[t]{1.2cm}{\raggedleft 2110\\(3.1\%)} & \parbox[t]{1.2cm}{\raggedleft 2741\\(3.5\%)} & \parbox[t]{1.2cm}{\raggedleft 3531\\(3.9\%)} & \parbox[t]{1.2cm}{\raggedleft 4473\\(4.5\%)} & \parbox[t]{1.6cm}{\raggedleft 15645\\(3.1\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION OTRO HOSPITAL DEL SERVICIO} & \parbox[t]{1.2cm}{\raggedleft 1631\\(1.7\%)} & \parbox[t]{1.2cm}{\raggedleft 1198\\(1.8\%)} & \parbox[t]{1.2cm}{\raggedleft 1069\\(1.6\%)} & \parbox[t]{1.2cm}{\raggedleft 1242\\(1.6\%)} & \parbox[t]{1.2cm}{\raggedleft 1671\\(1.8\%)} & \parbox[t]{1.2cm}{\raggedleft 2147\\(2.2\%)} & \parbox[t]{1.6cm}{\raggedleft 8958\\(1.8\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION OTRO HOSPITAL DE LA RED NACIONAL} & \parbox[t]{1.2cm}{\raggedleft 659\\(0.7\%)} & \parbox[t]{1.2cm}{\raggedleft 584\\(0.9\%)} & \parbox[t]{1.2cm}{\raggedleft 541\\(0.8\%)} & \parbox[t]{1.2cm}{\raggedleft 560\\(0.7\%)} & \parbox[t]{1.2cm}{\raggedleft 688\\(0.8\%)} & \parbox[t]{1.2cm}{\raggedleft 755\\(0.8\%)} & \parbox[t]{1.6cm}{\raggedleft 3787\\(0.8\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright ALTA VOLUNTARIA} & \parbox[t]{1.2cm}{\raggedleft 526\\(0.5\%)} & \parbox[t]{1.2cm}{\raggedleft 498\\(0.8\%)} & \parbox[t]{1.2cm}{\raggedleft 569\\(0.8\%)} & \parbox[t]{1.2cm}{\raggedleft 646\\(0.8\%)} & \parbox[t]{1.2cm}{\raggedleft 710\\(0.8\%)} & \parbox[t]{1.2cm}{\raggedleft 652\\(0.7\%)} & \parbox[t]{1.6cm}{\raggedleft 3601\\(0.7\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION A OTROS CENTROS (CARCEL)} & \parbox[t]{1.2cm}{\raggedleft 76\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 171\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 135\\(0.2\%)} & \parbox[t]{1.2cm}{\raggedleft 116\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 143\\(0.2\%)} & \parbox[t]{1.2cm}{\raggedleft 139\\(0.1\%)} & \parbox[t]{1.6cm}{\raggedleft 780\\(0.2\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION INST. PRIVADA (COMPRA DE SERVICIOS)} & \parbox[t]{1.2cm}{\raggedleft 144\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 97\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 112\\(0.2\%)} & \parbox[t]{1.2cm}{\raggedleft 91\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 142\\(0.2\%)} & \parbox[t]{1.2cm}{\raggedleft 167\\(0.2\%)} & \parbox[t]{1.6cm}{\raggedleft 753\\(0.2\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright FUGA DEL PACIENTE} & \parbox[t]{1.2cm}{\raggedleft 75\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 60\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 49\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 69\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 77\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 79\\(0.1\%)} & \parbox[t]{1.6cm}{\raggedleft 409\\(0.1\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION INST. PRIVADA (VOLUNTARIO)} & \parbox[t]{1.2cm}{\raggedleft 65\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 50\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 51\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 51\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 54\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 52\\(0.1\%)} & \parbox[t]{1.6cm}{\raggedleft 323\\(0.1\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright NO IDENTIFICADA} & \parbox[t]{1.2cm}{\raggedleft 4\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 1\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.6cm}{\raggedleft 5\\(0.0\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DESCONOCIDO} & \parbox[t]{1.2cm}{\raggedleft 1\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.6cm}{\raggedleft 1\\(0.0\%)} \\ \hline
\end{tabular}
\end{table}
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.4}
\caption{Distribución anual del tipo de alta para la cohorte control.}
\label{tab:frecuencia_tipo_alta_control}
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
\parbox[t]{4.0cm}{\centering\textbf{TIPO DE ALTA}} & 
\parbox[t]{1.2cm}{\centering\textbf{2019}} & 
...
\parbox[t]{1.2cm}{\centering\textbf{2024}} & 
\parbox[t]{1.6cm}{\centering\textbf{TOTAL}} \\ \hline
\parbox[t]{4.0cm}{\raggedright DOMICILIO} & \parbox[t]{1.2cm}{\raggedleft 964012\\(91.5\%)} & \parbox[t]{1.2cm}{\raggedleft 638565\\(89.1\%)} & \parbox[t]{1.2cm}{\raggedleft 657403\\(87.8\%)} & \parbox[t]{1.2cm}{\raggedleft 770531\\(90.1\%)} & \parbox[t]{1.2cm}{\raggedleft 857820\\(90.4\%)} & \parbox[t]{1.2cm}{\raggedleft 883172\\(89.5\%)} & \parbox[t]{1.6cm}{\raggedleft 4771503\\(89.9\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright FALLECIDO} & \parbox[t]{1.2cm}{\raggedleft 24263\\(2.3\%)} & \parbox[t]{1.2cm}{\raggedleft 25480\\(3.6\%)} & \parbox[t]{1.2cm}{\raggedleft 27542\\(3.7\%)} & \parbox[t]{1.2cm}{\raggedleft 22529\\(2.6\%)} & \parbox[t]{1.2cm}{\raggedleft 19822\\(2.1\%)} & \parbox[t]{1.2cm}{\raggedleft 20673\\(2.1\%)} & \parbox[t]{1.6cm}{\raggedleft 140309\\(2.6\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright HOSPITALIZACION DOMICILIARIA} & \parbox[t]{1.2cm}{\raggedleft 11668\\(1.1\%)} & \parbox[t]{1.2cm}{\raggedleft 12600\\(1.8\%)} & \parbox[t]{1.2cm}{\raggedleft 20293\\(2.7\%)} & \parbox[t]{1.2cm}{\raggedleft 20667\\(2.4\%)} & \parbox[t]{1.2cm}{\raggedleft 25767\\(2.7\%)} & \parbox[t]{1.2cm}{\raggedleft 32259\\(3.3\%)} & \parbox[t]{1.6cm}{\raggedleft 123254\\(2.3\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION OTRO HOSPITAL DEL SERVICIO} & \parbox[t]{1.2cm}{\raggedleft 23575\\(2.2\%)} & \parbox[t]{1.2cm}{\raggedleft 16641\\(2.3\%)} & \parbox[t]{1.2cm}{\raggedleft 17252\\(2.3\%)} & \parbox[t]{1.2cm}{\raggedleft 15246\\(1.8\%)} & \parbox[t]{1.2cm}{\raggedleft 18834\\(2.0\%)} & \parbox[t]{1.2cm}{\raggedleft 22776\\(2.3\%)} & \parbox[t]{1.6cm}{\raggedleft 114324\\(2.2\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright ALTA VOLUNTARIA} & \parbox[t]{1.2cm}{\raggedleft 9437\\(0.9\%)} & \parbox[t]{1.2cm}{\raggedleft 7071\\(1.0\%)} & \parbox[t]{1.2cm}{\raggedleft 8490\\(1.1\%)} & \parbox[t]{1.2cm}{\raggedleft 10075\\(1.2\%)} & \parbox[t]{1.2cm}{\raggedleft 9607\\(1.0\%)} & \parbox[t]{1.2cm}{\raggedleft 9999\\(1.0\%)} & \parbox[t]{1.6cm}{\raggedleft 54679\\(1.0\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION OTRO HOSPITAL DE LA RED NACIONAL} & \parbox[t]{1.2cm}{\raggedleft 7756\\(0.7\%)} & \parbox[t]{1.2cm}{\raggedleft 6287\\(0.9\%)} & \parbox[t]{1.2cm}{\raggedleft 6653\\(0.9\%)} & \parbox[t]{1.2cm}{\raggedleft 5979\\(0.7\%)} & \parbox[t]{1.2cm}{\raggedleft 6784\\(0.7\%)} & \parbox[t]{1.2cm}{\raggedleft 7183\\(0.7\%)} & \parbox[t]{1.6cm}{\raggedleft 40642\\(0.8\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION A OTROS CENTROS (CARCEL)} & \parbox[t]{1.2cm}{\raggedleft 2904\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 4225\\(0.6\%)} & \parbox[t]{1.2cm}{\raggedleft 3903\\(0.5\%)} & \parbox[t]{1.2cm}{\raggedleft 3269\\(0.4\%)} & \parbox[t]{1.2cm}{\raggedleft 3354\\(0.4\%)} & \parbox[t]{1.2cm}{\raggedleft 3606\\(0.4\%)} & \parbox[t]{1.6cm}{\raggedleft 21261\\(0.4\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright FUGA DEL PACIENTE} & \parbox[t]{1.2cm}{\raggedleft 4066\\(0.4\%)} & \parbox[t]{1.2cm}{\raggedleft 2659\\(0.4\%)} & \parbox[t]{1.2cm}{\raggedleft 2544\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 3115\\(0.4\%)} & \parbox[t]{1.2cm}{\raggedleft 3147\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 3220\\(0.3\%)} & \parbox[t]{1.6cm}{\raggedleft 18751\\(0.4\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION INST. PRIVADA (COMPRA DE SERVICIOS)} & \parbox[t]{1.2cm}{\raggedleft 4267\\(0.4\%)} & \parbox[t]{1.2cm}{\raggedleft 2333\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 3576\\(0.5\%)} & \parbox[t]{1.2cm}{\raggedleft 2282\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 2744\\(0.3\%)} & \parbox[t]{1.2cm}{\raggedleft 2698\\(0.3\%)} & \parbox[t]{1.6cm}{\raggedleft 17900\\(0.3\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DERIVACION INST. PRIVADA (VOLUNTARIO)} & \parbox[t]{1.2cm}{\raggedleft 1549\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 1102\\(0.2\%)} & \parbox[t]{1.2cm}{\raggedleft 1327\\(0.2\%)} & \parbox[t]{1.2cm}{\raggedleft 1075\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 993\\(0.1\%)} & \parbox[t]{1.2cm}{\raggedleft 1074\\(0.1\%)} & \parbox[t]{1.6cm}{\raggedleft 7120\\(0.1\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright NO IDENTIFICADA} & \parbox[t]{1.2cm}{\raggedleft 54\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 21\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.6cm}{\raggedleft 75\\(0.0\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DESCONOCIDO} & \parbox[t]{1.2cm}{\raggedleft 18\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 1\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.6cm}{\raggedleft 19\\(0.0\%)} \\ \hline
\end{tabular}
\end{table}

En segundo lugar, la variable de \textbf{severidad} hospitalaria (\texttt{IR\_29301\_SEVERIDAD}) es calculada por el agrupador IR-GRD a partir del diagnóstico principal, los diagnósticos secundarios, los procedimientos y otras características clínicas del episodio, expresándose en cuatro niveles ordinales: 0 (sin gravedad), 1 (menor), 2 (moderada) y 3 (mayor).

Como se puede observar en las Tablas \ref{tab:frecuencia_severidad_total}, \ref{tab:frecuencia_severidad_onco} y \ref{tab:frecuencia_severidad_control}, esta variable presenta una cardinalidad baja de cuatro categorías. Además, posee una proporción de valores nulos y desconocidos cercana al 0\% (89 casos en total durante todo el período analizado), lo que la convierte en una variable objetivo de alta calidad para el modelado. Por ello, se decidió conservarla como una variable ordinal, sin necesidad de binarizarla ni transformarla.

Específicamente, en el análisis de frecuencias se observa que, mientras en la cohorte control la mayor proporción de ingresos recae históricamente en la severidad menor (nivel 1), alcanzando hasta un 50,5\% en 2019, los pacientes oncológicos demandan niveles de complejidad clínica más elevados, con una mayor proporción de ingresos en los niveles de severidad moderada (nivel 2) y mayor (nivel 3), alcanzando hasta un 31,4\% y un 20,2\%, respectivamente, en 2019. En otras palabras, los datos evidencian que los episodios de cáncer se concentran hacia los niveles superiores de la escala de severidad clínica, ya que la severidad de nivel 2 (moderada) constituye la categoría modal de esta cohorte.

Respecto del comportamiento de la severidad mayor (nivel 3) en la cohorte oncológica, se puede observar que sufrió una escalada progresiva a partir de 2020, pasando de un 20,2\% en 2019 a representar casi un tercio de las hospitalizaciones (30,5\%) al cierre de 2024. Este desplazamiento hacia los estratos de mayor gravedad podría indicar que las camas de la red oncológica están siendo ocupadas por pacientes con cuadros clínicos cada vez más deteriorados.
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.4}
\caption{Distribución anual de severidad (IR\_29301\_SEVERIDAD) para la cohorte total.}
\label{tab:frecuencia_severidad_total}
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
\parbox[t]{4.0cm}{\centering\textbf{SEVERIDAD}} & 
\parbox[t]{1.2cm}{\centering\textbf{2019}} & 
\parbox[t]{1.2cm}{\centering\textbf{2020}} & 
\parbox[t]{1.2cm}{\centering\textbf{2021}} & 
\parbox[t]{1.2cm}{\centering\textbf{2022}} & 
...
\parbox[t]{1.2cm}{\centering\textbf{2024}} & 
\parbox[t]{1.6cm}{\centering\textbf{TOTAL}} \\ \hline
\parbox[t]{4.0cm}{\raggedright Nivel 1 (Menor)} & \parbox[t]{1.2cm}{\raggedleft 558571\\(48.5\%)} & \parbox[t]{1.2cm}{\raggedleft 366293\\(46.8\%)} & \parbox[t]{1.2cm}{\raggedleft 340136\\(41.6\%)} & \parbox[t]{1.2cm}{\raggedleft 373440\\(40.0\%)} & \parbox[t]{1.2cm}{\raggedleft 388694\\(37.4\%)} & \parbox[t]{1.2cm}{\raggedleft 387258\\(35.7\%)} & \parbox[t]{1.6cm}{\raggedleft 2414392\\(41.6\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright Nivel 2 (Moderada)} & \parbox[t]{1.2cm}{\raggedleft 242291\\(21.0\%)} & \parbox[t]{1.2cm}{\raggedleft 176554\\(22.6\%)} & \parbox[t]{1.2cm}{\raggedleft 183822\\(22.5\%)} & \parbox[t]{1.2cm}{\raggedleft 213260\\(22.9\%)} & \parbox[t]{1.2cm}{\raggedleft 253086\\(24.3\%)} & \parbox[t]{1.2cm}{\raggedleft 268865\\(24.8\%)} & \parbox[t]{1.6cm}{\raggedleft 1337878\\(23.0\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright Nivel 3 (Mayor)} & \parbox[t]{1.2cm}{\raggedleft 147998\\(12.9\%)} & \parbox[t]{1.2cm}{\raggedleft 159400\\(20.4\%)} & \parbox[t]{1.2cm}{\raggedleft 190013\\(23.3\%)} & \parbox[t]{1.2cm}{\raggedleft 191342\\(20.5\%)} & \parbox[t]{1.2cm}{\raggedleft 204227\\(19.6\%)} & \parbox[t]{1.2cm}{\raggedleft 217624\\(20.0\%)} & \parbox[t]{1.6cm}{\raggedleft 1110604\\(19.1\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright Nivel 0 (Sin gravedad)} & \parbox[t]{1.2cm}{\raggedleft 202517\\(17.6\%)} & \parbox[t]{1.2cm}{\raggedleft 79664\\(10.2\%)} & \parbox[t]{1.2cm}{\raggedleft 102938\\(12.6\%)} & \parbox[t]{1.2cm}{\raggedleft 154772\\(16.6\%)} & \parbox[t]{1.2cm}{\raggedleft 193550\\(18.6\%)} & \parbox[t]{1.2cm}{\raggedleft 212051\\(19.5\%)} & \parbox[t]{1.6cm}{\raggedleft 945492\\(16.3\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DESCONOCIDO} & \parbox[t]{1.2cm}{\raggedleft 21\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 23\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 30\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 15\\(0.0\%)} & \parbox[t]{1.6cm}{\raggedleft 89\\(0.0\%)} \\ \hline
\end{tabular}
\end{table}
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.4}
\caption{Distribución anual de severidad (IR\_29301\_SEVERIDAD) para la cohorte oncológica.}
\label{tab:frecuencia_severidad_onco}
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
\parbox[t]{4.0cm}{\centering\textbf{SEVERIDAD}} & 
\parbox[t]{1.2cm}{\centering\textbf{2019}} & 
...
\parbox[t]{1.2cm}{\centering\textbf{2024}} & 
\parbox[t]{1.6cm}{\centering\textbf{TOTAL}} \\ \hline
\parbox[t]{4.0cm}{\raggedright Nivel 2 (Moderada)} & \parbox[t]{1.2cm}{\raggedleft 30680\\(31.4\%)} & \parbox[t]{1.2cm}{\raggedleft 23807\\(36.7\%)} & \parbox[t]{1.2cm}{\raggedleft 23718\\(34.9\%)} & \parbox[t]{1.2cm}{\raggedleft 26710\\(34.2\%)} & \parbox[t]{1.2cm}{\raggedleft 31917\\(35.2\%)} & \parbox[t]{1.2cm}{\raggedleft 34607\\(34.9\%)} & \parbox[t]{1.6cm}{\raggedleft 171439\\(34.4\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright Nivel 3 (Mayor)} & \parbox[t]{1.2cm}{\raggedleft 19771\\(20.2\%)} & \parbox[t]{1.2cm}{\raggedleft 18413\\(28.4\%)} & \parbox[t]{1.2cm}{\raggedleft 19393\\(28.6\%)} & \parbox[t]{1.2cm}{\raggedleft 22960\\(29.4\%)} & \parbox[t]{1.2cm}{\raggedleft 26813\\(29.6\%)} & \parbox[t]{1.2cm}{\raggedleft 30265\\(30.5\%)} & \parbox[t]{1.6cm}{\raggedleft 137615\\(27.6\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright Nivel 1 (Menor)} & \parbox[t]{1.2cm}{\raggedleft 26703\\(27.3\%)} & \parbox[t]{1.2cm}{\raggedleft 19058\\(29.4\%)} & \parbox[t]{1.2cm}{\raggedleft 19126\\(28.2\%)} & \parbox[t]{1.2cm}{\raggedleft 20488\\(26.2\%)} & \parbox[t]{1.2cm}{\raggedleft 21770\\(24.0\%)} & \parbox[t]{1.2cm}{\raggedleft 23198\\(23.4\%)} & \parbox[t]{1.6cm}{\raggedleft 130343\\(26.1\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright Nivel 0 (Sin gravedad)} & \parbox[t]{1.2cm}{\raggedleft 20675\\(21.1\%)} & \parbox[t]{1.2cm}{\raggedleft 3648\\(5.6\%)} & \parbox[t]{1.2cm}{\raggedleft 5689\\(8.4\%)} & \parbox[t]{1.2cm}{\raggedleft 7909\\(10.1\%)} & \parbox[t]{1.2cm}{\raggedleft 10215\\(11.3\%)} & \parbox[t]{1.2cm}{\raggedleft 11081\\(11.2\%)} & \parbox[t]{1.6cm}{\raggedleft 59217\\(11.9\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DESCONOCIDO} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 2\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 2\\(0.0\%)} & \parbox[t]{1.6cm}{\raggedleft 4\\(0.0\%)} \\ \hline
\end{tabular}
\end{table}
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.4}
\caption{Distribución anual de severidad (IR\_29301\_SEVERIDAD) para la cohorte control.}
\label{tab:frecuencia_severidad_control}
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
\parbox[t]{4.0cm}{\centering\textbf{SEVERIDAD}} & 
\parbox[t]{1.2cm}{\centering\textbf{2019}} & 
...
\parbox[t]{1.2cm}{\centering\textbf{2024}} & 
\parbox[t]{1.6cm}{\centering\textbf{TOTAL}} \\ \hline
\parbox[t]{4.0cm}{\raggedright Nivel 1 (Menor)} & \parbox[t]{1.2cm}{\raggedleft 531868\\(50.5\%)} & \parbox[t]{1.2cm}{\raggedleft 347235\\(48.4\%)} & \parbox[t]{1.2cm}{\raggedleft 321010\\(42.9\%)} & \parbox[t]{1.2cm}{\raggedleft 352952\\(41.3\%)} & \parbox[t]{1.2cm}{\raggedleft 366924\\(38.7\%)} & \parbox[t]{1.2cm}{\raggedleft 364060\\(36.9\%)} & \parbox[t]{1.6cm}{\raggedleft 2284049\\(43.0\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright Nivel 2 (Moderada)} & \parbox[t]{1.2cm}{\raggedleft 211611\\(20.1\%)} & \parbox[t]{1.2cm}{\raggedleft 152747\\(21.3\%)} & \parbox[t]{1.2cm}{\raggedleft 160104\\(21.4\%)} & \parbox[t]{1.2cm}{\raggedleft 186550\\(21.8\%)} & \parbox[t]{1.2cm}{\raggedleft 221169\\(23.3\%)} & \parbox[t]{1.2cm}{\raggedleft 234258\\(23.7\%)} & \parbox[t]{1.6cm}{\raggedleft 1166439\\(22.0\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright Nivel 3 (Mayor)} & \parbox[t]{1.2cm}{\raggedleft 128227\\(12.2\%)} & \parbox[t]{1.2cm}{\raggedleft 140987\\(19.7\%)} & \parbox[t]{1.2cm}{\raggedleft 170620\\(22.8\%)} & \parbox[t]{1.2cm}{\raggedleft 168382\\(19.7\%)} & \parbox[t]{1.2cm}{\raggedleft 177414\\(18.7\%)} & \parbox[t]{1.2cm}{\raggedleft 187359\\(19.0\%)} & \parbox[t]{1.6cm}{\raggedleft 972989\\(18.3\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright Nivel 0 (Sin gravedad)} & \parbox[t]{1.2cm}{\raggedleft 181842\\(17.3\%)} & \parbox[t]{1.2cm}{\raggedleft 76016\\(10.6\%)} & \parbox[t]{1.2cm}{\raggedleft 97249\\(13.0\%)} & \parbox[t]{1.2cm}{\raggedleft 146863\\(17.2\%)} & \parbox[t]{1.2cm}{\raggedleft 183335\\(19.3\%)} & \parbox[t]{1.2cm}{\raggedleft 200970\\(20.4\%)} & \parbox[t]{1.6cm}{\raggedleft 886275\\(16.7\%)} \\ \hline
\parbox[t]{4.0cm}{\raggedright DESCONOCIDO} & \parbox[t]{1.2cm}{\raggedleft 21\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 0\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 21\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 30\\(0.0\%)} & \parbox[t]{1.2cm}{\raggedleft 13\\(0.0\%)} & \parbox[t]{1.6cm}{\raggedleft 85\\(0.0\%)} \\ \hline
\end{tabular}
\end{table}

En resumen, se tomaron las siguientes decisiones para cada variable categórica objetivo:
\begin{enumerate}
    \item \textbf{Mortalidad (\texttt{TIPOALTA}):} Se decidió \textbf{binarizar} la variable en un desenlace dicotómico que indique si el paciente falleció (1) o no (0), descartando las categorías marginales sin información (\texttt{NO IDENTIFICADA} y \texttt{DESCONOCIDO}).
    \item \textbf{Severidad (\texttt{IR\_29301\_SEVERIDAD}):} Se decidió \textbf{mantener} la variable como ordinal, sin necesidad de binarizarla ni transformarla, ya que presenta una cardinalidad baja de cuatro categorías y una proporción de valores nulos y desconocidos cercana al 0\%, los cuales serán imputados directamente.
\end{enumerate}

\subsection{Juicio sobre las de variables categóricas de entrada para la fase de modelado}
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.4}
\caption{Características y decisión de preprocesamiento de las variables categóricas para la fase de modelado.}
\begin{tabular}{|c|c|c|c|c|c|}
\hline
\parbox[t]{2.6cm}{\centering\textbf{Variable}} &
\parbox[t]{1.5cm}{\centering\textbf{Tipo}} &
\parbox[t]{1cm}{\centering\textbf{Cardina-\\lidad}} &
\parbox[t]{4.4cm}{\centering\textbf{Categorías}} &
\parbox[t]{1.4cm}{\centering\textbf{Nulos}} &
\parbox[t]{3cm}{\centering\textbf{Decisión}} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{SEXO}} &
\parbox[t]{1.5cm}{\centering Demográfica} &
\parbox[t]{1cm}{\centering 2} &
\parbox[t]{4.4cm}{\raggedright Mujer, Hombre} &
\parbox[t]{1.4cm}{\centering 559\\(0\%)} &
\parbox[t]{3cm}{\raggedright Retener} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{ETNIA}} &
\parbox[t]{1.5cm}{\centering Demográfica} &
\parbox[t]{1cm}{\centering 12} &
\parbox[t]{4.4cm}{\raggedright Ninguno, Otro, Mapuche, Aymara, Pascuense, Diaguita, Kawésqar, Quechua, Yagán, Colla, Lickanantay \ldots} &
\parbox[t]{1.4cm}{\centering 19\\(0\%)} &
\parbox[t]{3cm}{\raggedright Binarizar (pueblo originario)} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{PROVINCIA}} &
\parbox[t]{1.5cm}{\centering Demográfica} &
\parbox[t]{1cm}{\centering 57} &
\parbox[t]{4.4cm}{\raggedright Santiago, Concepción, Cautín, Cordillera, Valparaíso, Elqui, Biobío \ldots} &
\parbox[t]{1.4cm}{\centering 2.208\\(0\%)} &
\parbox[t]{3cm}{\raggedright Agrupar (regiones)} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{COMUNA}} &
\parbox[t]{1.5cm}{\centering Demográfica} &
\parbox[t]{1cm}{\centering 346} &
\parbox[t]{4.4cm}{\raggedright Puente Alto, La Florida, Maipú, Valparaíso, Arica, Antofagasta \ldots} &
\parbox[t]{1.4cm}{\centering 1.261\\(0\%)} &
\parbox[t]{3cm}{\raggedright Descartar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{NACIONALIDAD}} &
\parbox[t]{1.5cm}{\centering Demográfica} &
\parbox[t]{1cm}{\centering 214} &
\parbox[t]{4.4cm}{\raggedright Chile, Venezuela, Perú, Bolivia, Haití, Colombia, Ecuador \ldots} &
\parbox[t]{1.4cm}{\centering 23.677\\(0,4\%)} &
\parbox[t]{3cm}{\raggedright Binarizar (extranjero)} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{PREVISION}} &
\parbox[t]{1.5cm}{\centering Demográfica} &
\parbox[t]{1cm}{\centering 14} &
\parbox[t]{4.4cm}{\raggedright FONASA institucional (MAI A, B, C y D), FONASA libre elección B (\texttt{FMLE\_B}), C (\texttt{FMLE\_C}) y D (\texttt{FMLE\_D}), ISAPRE, Particular, DIPRECA, CAPREDENA, Caja de Previsión FFAA (SISA) \ldots} &
\parbox[t]{1.4cm}{\centering 142\\(0\%)} &
\parbox[t]{3cm}{\raggedright Agrupar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{SERVICIO\_SALUD}} &
\parbox[t]{1.5cm}{\centering Hospitalaria} &
\parbox[t]{1cm}{\centering 31} &
\parbox[t]{4.4cm}{\raggedright Metropolitano Suroriente, Del Maule, Metropolitano Occidente, Metropolitano Sur, Metropolitano Central \ldots} &
\parbox[t]{1.4cm}{\centering 1.399\\(0\%)} &
\parbox[t]{3cm}{\raggedright Agrupar (macrozonas)} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{TIPO\_PROCEDENCIA}} &
\parbox[t]{1.5cm}{\centering Hospitalaria} &
\parbox[t]{1cm}{\centering 19} &
\parbox[t]{4.4cm}{\raggedright Servicio de emergencia (domicilio), Centro de especialidades, Otros hospitales de la red, APS urgencia, CESFAM \ldots} &
\parbox[t]{1.4cm}{\centering 3.111\\(0,1\%)} &
\parbox[t]{3cm}{\raggedright Agrupar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{TIPO\_INGRESO}} &
\parbox[t]{1.5cm}{\centering Hospitalaria} &
\parbox[t]{1cm}{\centering 5} &
\parbox[t]{4.4cm}{\raggedright Urgencia, Programada, Obstétrica, No identificada, No programada} &
\parbox[t]{1.4cm}{\centering 145\\(0\%)} &
\parbox[t]{3cm}{\raggedright Agrupar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{ESPECIALIDAD\_MEDICA}} &
\parbox[t]{1.5cm}{\centering Hospitalaria} &
\parbox[t]{1cm}{\centering 102} &
\parbox[t]{4.4cm}{\raggedright Obstetricia y ginecología, Cirugía general, Medicina interna, Traumatología y ortopedia \ldots} &
\parbox[t]{1.4cm}{\centering 54\\(0\%)} &
\parbox[t]{3cm}{\raggedright Agrupar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{TIPO\_ACTIVIDAD}} &
\parbox[t]{1.5cm}{\centering Hospitalaria} &
\parbox[t]{1cm}{\centering 5} &
\parbox[t]{4.4cm}{\raggedright Hospitalización, Cirugía mayor ambulatoria (CMA), Hospitalización en urgencia, Hospitalización diurna \ldots} &
\parbox[t]{1.4cm}{\centering 18\\(0\%)} &
\parbox[t]{3cm}{\raggedright Agrupar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{SERVICIOINGRESO}} &
\parbox[t]{1.5cm}{\centering Hospitalaria} &
\parbox[t]{1cm}{\centering 92} &
\parbox[t]{4.4cm}{\raggedright Cirugía, Unidad de recuperación de pabellones, Medicina, Obstetricia, Pediatría, Oncología \ldots} &
\parbox[t]{1.4cm}{\centering 27.843\\(0,5\%)} &
\parbox[t]{3cm}{\raggedright Agrupar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{SERVICIOTRASLADO1-9}} &
\parbox[t]{1.5cm}{\centering Hospitalaria} &
\parbox[t]{1cm}{\centering 97 a 66} &
\parbox[t]{4.4cm}{\raggedright Medicina, Unidad de tratamiento intermedio adulto, Cirugía \ldots} &
\parbox[t]{1.4cm}{\centering Desde 4.807.469\\(82,8\%)} &
\parbox[t]{3cm}{\raggedright Descartar y derivar a \texttt{CANTIDAD\_TRASLADOS}} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{SERVICIOALTA}} &
\parbox[t]{1.5cm}{\centering Hospitalaria} &
\parbox[t]{1cm}{\centering 89} &
\parbox[t]{4.4cm}{\raggedright Cirugía, Medicina, Unidad de recuperación de pabellones, Obstetricia, Pediatría, Oncología \ldots} &
\parbox[t]{1.4cm}{\centering 24.588\\(0,4\%)} &
\parbox[t]{3cm}{\raggedright Descartar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{CONDICIONDEALTA\\NEONATO1-4}} &
\parbox[t]{1.5cm}{\centering Clínica} &
\parbox[t]{1cm}{\centering 2 a 0} &
\parbox[t]{4.4cm}{\raggedright Vivo, Fallecido} &
\parbox[t]{1.4cm}{\centering Desde 5.679.510\\(97,8\%)} &
\parbox[t]{3cm}{\raggedright Descartar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{SEXORN1-4}} &
\parbox[t]{1.5cm}{\centering Demográfica} &
\parbox[t]{1cm}{\centering 2 a 1} &
\parbox[t]{4.4cm}{\raggedright Hombre, Mujer, Nulos} &
\parbox[t]{1.4cm}{\centering Desde 5.161.114\\(88,9\%)} &
\parbox[t]{3cm}{\raggedright Descartar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{RN1ESTADO-4}} &
\parbox[t]{1.5cm}{\centering Hospitalaria} &
\parbox[t]{1cm}{\centering 13 a 2} &
\parbox[t]{4.4cm}{\raggedright Nulos, 0, 9, 10, 8, 7 \ldots} &
\parbox[t]{1.4cm}{\centering Desde 3.888.841\\(67\%)} &
\parbox[t]{3cm}{\raggedright Descartar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{DIAGNOSTICO1-35}} &
\parbox[t]{1.5cm}{\centering Clínica} &
\parbox[t]{1cm}{\centering 15.300 a 799} &
\parbox[t]{4.4cm}{\raggedright H26.9, K80.2, C50.9, C73, C16.9 \ldots} &
\parbox[t]{1.4cm}{\centering Desde 11.316\\(0,2\%)} &
\parbox[t]{3cm}{\raggedright Generar las variables derivadas: categoría de cáncer, tipo de diagnóstico oncológico, carga oncológica, número de comorbilidades y comorbilidad principal} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{PROCEDIMIENTO1-30}} &
\parbox[t]{1.5cm}{\centering Hospitalaria} &
\parbox[t]{1cm}{\centering 3.999 a 762} &
\parbox[t]{4.4cm}{\raggedright 73.59, 74.1, 99.25, 86.07, 88.01 \ldots} &
\parbox[t]{1.4cm}{\centering Desde 35\\(0\%)} &
\parbox[t]{3cm}{\raggedright Mantener \texttt{PROCEDIMIENTO1} y generar la variable \texttt{NUMERO\_PROCEDIMIENTOS}} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{ESPECIALIDAD\\INTERVENCION}} &
\parbox[t]{1.5cm}{\centering Hospitalaria} &
\parbox[t]{1cm}{\centering 103} &
\parbox[t]{4.4cm}{\raggedright Cirugía general, Obstetricia y ginecología, Traumatología y ortopedia, Oftalmología, Urología \ldots} &
\parbox[t]{1.4cm}{\centering 2.602.138\\(44,8\%)} &
\parbox[t]{3cm}{\raggedright Descartar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{USOSPABELLON}} &
\parbox[t]{1.5cm}{\centering Hospitalaria} &
\parbox[t]{1cm}{\centering 71} &
\parbox[t]{4.4cm}{\raggedright 1, 2, 3, 4, 5, 6, 7 \ldots} &
\parbox[t]{1.4cm}{\centering 2.847.183\\(49\%)} &
\parbox[t]{3cm}{\raggedright Descartar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{IR\_29301\_COD\_GRD}} &
\parbox[t]{1.5cm}{\centering Hospitalaria} &
\parbox[t]{1cm}{\centering 1.671} &
\parbox[t]{4.4cm}{\raggedright 022360, 146101, 174132, 174133, 174131 \ldots} &
\parbox[t]{1.4cm}{\centering 89\\(0\%)} &
\parbox[t]{3cm}{\raggedright Descartar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{IR\_29301\_MORTALIDAD}} &
\parbox[t]{1.5cm}{\centering Clínica} &
\parbox[t]{1cm}{\centering 4} &
\parbox[t]{4.4cm}{\raggedright 0 (Sin gravedad), 1 (Menor), 2 (Moderada), 3 (Mayor)} &
\parbox[t]{1.4cm}{\centering 89\\(0\%)} &
\parbox[t]{3cm}{\raggedright Descartar} \\ \hline

\parbox[t]{2.6cm}{\raggedright \texttt{HOSPPROCEDENCIA}} &
\parbox[t]{1.5cm}{\centering Hospitalaria} &
\parbox[t]{1cm}{\centering 518} &
\parbox[t]{4.4cm}{\raggedright Hospital San Pablo (Coquimbo), Hospital San José (Coronel), Hospital Carlos Van Buren (Valparaíso) \ldots} &
\parbox[t]{1.4cm}{\centering 5.104.603\\(87,9\%)} &
\parbox[t]{3cm}{\raggedright Descartar} \\ \hline
